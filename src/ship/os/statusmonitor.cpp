...
void shipos::StatusMonitor::render(ConsoleKey key) {
  ...
  if (hull_ratio < 0.1) {
    wattron(this->win, A_BLINK | A_BOLD | COLOR_PAIR((int)ConsoleColor::RED));
  } else if (hull_ratio < 0.3) {
    wattron(this->win, A_BOLD | COLOR_PAIR((int)ConsoleColor::YELLOW));
  }
  ...
  if (hull_ratio < 0.1) {
    wattroff(this->win, A_BLINK | A_BOLD | COLOR_PAIR((int)ConsoleColor::RED));
  } else if (hull_ratio < 0.3) {
    wattroff(this->win, A_BOLD | COLOR_PAIR((int)ConsoleColor::YELLOW));
  }
